...
namespace Reloj_Marcador.Services
{
    public class FuncionariosServices : IFuncionariosService
    {
        ...
        public async Task<int> CrearAsync(Funcionarios_Usuarios funcionario)
        {
            ...
            if (resultado > 0)
            {
                try
                {
                    ...
                    await _bitacoraService.RegistrarAsync(usuario, "Se creó funcionario", funcionario);
                }
                catch (Exception ex)
                {
        ...
        public async Task<string> Crear_ContrasenaAsync()
            => await _funcionarioRepository.Crear_ContrasenaAsync();
        ...
        public async Task<(bool Resultado, string Mensaje)> Cambiar_ContrasenaAsync(string identificacion, string contrasena)
        {
            ...
            try
            {
                ...
                if (resultado > 0)
                {
                    ...
                    {
                        string usuario = _httpContextAccessor.HttpContext?.User?.Identity?.Name ?? "Anónimo";
                        await _bitacoraService.RegistrarAsync(usuario, "Cambio de contraseña", new { Identificacion = identificacion });
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"Error registrando bitácora: {ex.Message}");
        ...
        public async Task<IEnumerable<Funcionarios_Areas>> ListarFuncionariosAreasAsync(string identificacion)
            => await _funcionarioRepository.ListarFuncionariosAreasAsync(identificacion);

        public async Task<IEnumerable<Funcionarios_Areas>> ListarAreasAsync()
            => await _funcionarioRepository.ListarAreasAsync();
        ...
        public async Task<(bool resultado, string mensaje)> InsertarFuncionarioAreaAsync(Funcionarios_Areas funcionarioArea)
        {
            try
            {
                ...
                if (resultado.resultado)
                {
                    try
                    {
                        var lista = await _funcionarioRepository.ListarFuncionariosAreasAsync(funcionarioArea.Identificacion);

                        var registroCompleto = lista.FirstOrDefault(a => a.ID_Area == funcionarioArea.ID_Area);

                        string usuario = _httpContextAccessor.HttpContext?.User?.Identity?.Name ?? "Anónimo";

                        await _bitacoraService.RegistrarAsync(usuario, "Se asignó funcionario a un área",
                            registroCompleto ?? funcionarioArea);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"Error registrando bitácora: {ex.Message}");
        ...
        public async Task<(bool Resultado, string Mensaje)> EliminarAsociacionAsync(Funcionarios_Areas funcionarioArea)
        {
            ...
            {
                var lista = await _funcionarioRepository.ListarFuncionariosAreasAsync(funcionarioArea.Identificacion ?? "");
                var registroAEliminar = lista.FirstOrDefault(a => a.ID_Funcionario_Area == funcionarioArea.ID_Funcionario_Area);
                ...
                if (resultado.Resultado)
                {
                    try
                    {
                        ...
                        await _bitacoraService.RegistrarAsync(usuario, "Se eliminó asociación de un funcionario a un área",
                            registroAEliminar ?? funcionarioArea);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"Error registrando bitácora: {ex.Message}");
