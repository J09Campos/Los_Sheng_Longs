...
{
    public class Tipos_InconsistenciasRepository
    {
        private readonly IDbConnectionFactory _dbConnectionFactory;

        public Tipos_InconsistenciasRepository(IDbConnectionFactory dbConnectionFactory)
        {
            _dbConnectionFactory = dbConnectionFactory;
        }

        public async Task<IEnumerable<Tipos_Inconsistencia>> GetAllAsync()
        {
            using (var connection = _dbConnectionFactory.CreateConnection())
            {
                ...
                var lista = await connection.QueryAsync<Tipos_Inconsistencia>(
        ...
        public async Task<Tipos_Inconsistencia?> GetByIdAsync(string id)
        {
            using (var connection = _dbConnectionFactory.CreateConnection())
            {
                var parametros = new DynamicParameters();
        ...
        public async Task<(bool Resultado, string Mensaje)> CRUDAsync(Tipos_Inconsistencia inconsistencia, string accion)
        {
            using (var connection = _dbConnectionFactory.CreateConnection())
            {
                var parametros = new DynamicParameters();
